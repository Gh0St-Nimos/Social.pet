...
$q = $c->prepare("SELECT * FROM actions
    ...
    INNER JOIN users ON users.id = posts.userid
         WHERE actions.userid = ? AND actions.type = 'save' GROUP BY posts.id");
				...
				foreach ($posts as $post) {
                    ...
                    <div class="postinho">
                        <div class="post-details">
                            ...
                            <span class="username"><?= $post["username"] ?> — <small onclick="window.location.href='./perfil.php?perfil=<?= $post["userlogin"] ?>'">@<?= $post["userlogin"] ?></small> </span>
                        </div>
                        <div class="post-actiono">
                            <i class="fas fa-star"></i>
                            <p class="curtido">Você salvou essa publicação</p>
                        </div>
                    </div>
